      ...
      <div className="screen" style={{ padding: '0 0 100px' }}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 30px', textAlign: 'center', gap: 16 }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--text-dim)', lineHeight: 1.7 }}>Start your collection by scanning a wine bottle label.</p>
            <button className="btn btn-primary" onClick={() => navigate('/add')}>Add Your First Wine</button>
          </div>
        ) : (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, padding: '0 14px' }}>
              {wines.map(w => <WineCard key={w.id} wine={w} onClick={() => navigate(`/wine/${w.id}`)} />)}
            </div>
          </>
        )}
      </div>
